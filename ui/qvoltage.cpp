...
void QVoltageWidget::createConnections() {
    ...
    connect(mView, &VoltageView::stat, this, [this](auto status, auto type, VoltageView::Stat stat) {
        if(!status.isEmpty())
            QMessageBox::critical(this, "stat", status);
        else
            mState[type]->setChecked(stat.gs());
    });
    connect(mView, &VoltageView::setVoltage, this, [this](auto status) {
        if(!status.isEmpty())
            QMessageBox::critical(this, "setVoltage", status);
    });
    connect(mView, &VoltageView::setSpeedUp, this, [this](auto status) {
        if(!status.isEmpty())
            QMessageBox::critical(this, "setSpeedUp", status);
    });
    connect(mView, &VoltageView::setSpeedDn, this, [this](auto status) {
        if(!status.isEmpty())
            QMessageBox::critical(this, "setSpeedDn", status);
    });
    connect(mView, &VoltageView::turnOn, this, [this](auto status) {
        if(!status.isEmpty())
            QMessageBox::critical(this, "turnOn", status);
    });
    connect(mView, &VoltageView::turnOff, this, [this](auto status) {
        if(!status.isEmpty())
            QMessageBox::critical(this, "turnOff", status);
    });
    connect(mView, &VoltageView::open, this, [this](auto status) {
        if(!status.isEmpty())
            QMessageBox::critical(this, "open", status);
    });
    connect(mView, &VoltageView::close, this, [this](auto status) {
        if(!status.isEmpty())
            QMessageBox::critical(this, "close", status);
    });
    connect(mView, &VoltageView::isOpen, this, [this](auto status, auto flag) {
        if(!status.isEmpty())
            QMessageBox::critical(this, "isOpen", status);
        if(flag)
            mOpen->setText("Close");
        else
            mOpen->setText("Open");
